...
have to give it a condition not just variable
...
Create a new variable and modify existing variable
...
**correct form** mutate( litters_df,  
...
Pipe function: data processing pipeline (missing mutate)
...
**correct way:** litters_df read_csv(“./data/FAS_litters.csv”) %\>%
janitor::clean_names() %\>% select(-pup_survive) %\>% mutate(wt_gain =
gd18_weight - gd0_weight) %\>% drop_na(gd0_weight)
